...
class ChartRenderer:
    ...
    def render_chromatogram(peaks: List[Dict[str, Any]], anomalies: List[Dict[str, Any]]) -> bytes:
        ...
        if not peaks:
            ...
            rts = [float(p.get("retention_time", 0.0)) for p in peaks]
            intensities = [float(p.get("intensity", 0.0)) for p in peaks]
        ...
        ax.set_title("Chromatogram & Peak Profile", fontsize=11, fontweight="bold", pad=10)
    ...
    def render_anomaly_distribution(anomalies: List[Dict[str, Any]]) -> bytes:
        ...
        if not anomalies:
            ...
            pids = [a.get("peak_id", "") for a in anomalies]
            scores = [float(a.get("anomaly_score", 0.0)) for a in anomalies]
            colors = [
